...
serversocket.bind(ip_port)  #绑定到IP地址和端口号

serversocket.listen(1)   #开始侦听，队列长度为1
clientsocket,clientaddress = serversocket.accept() #等待客户机连接请求
...
while 1:  #循环以接收和回送客户机
    ...
    print('Recieved from client:',repr(data)) #输出接收到的数据，repr函数转换为字符串
    print('Echo :',repr(data))   #输出发送到客户机的数据信息
